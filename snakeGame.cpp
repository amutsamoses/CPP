...
class Snake
{
	int p1, p2, v1, v2, v3, e1, e2;
	int prev, now, n, colr, dsp, cnt, dly, m;
	int stp, egGen;
	int xr, yr;
	int nosound(), sound;
	void caught();
...
void Snake::show()
{
	...
	{
		fillellipse(pos[0][9], pos[0][1], yr, xr);
		setcolor(x);
	}
}
		...
		while(1)
		{
			...
			else
			{
				if (twnkl % 2)
				{
					clr = getcolor();
					setcolor(10);
					rectangle(endX+40, endY-10, getmaxx()-35, getmaxx()-160);
					outtextxy(endX+60, endY-29, "PAUSE");
					setcolor(clr);
					delay(1000);
				}
